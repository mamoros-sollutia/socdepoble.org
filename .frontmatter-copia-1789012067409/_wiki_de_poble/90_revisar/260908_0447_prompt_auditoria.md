...
Auditoria Final de Puresa i Seguretat (IAs Gratuïtes: Gemini, Deepseek, Qwen, Kimi, Grok).
Les vulnerabilitats crítiques (CSP, XSS, Umami, RLS del Mur tancat) ja s'han sanejat. L'objectiu ara és buscar vectors menors, deute tècnic "code smells" i confirmar si el sistema és segur per obrir-lo en BETA a un grup reduït de WhatsApp demà.

`OBJECTIU: Auditar la puresa final del codi amb evidències verificables abans d'obrir la beta.`

## Context Necessari

Bundle aparellat: 260908_0447_BUNDLE_auditoria.md.
...
Ets una de les IAs del Consell (Gemini, Deepseek, Qwen, Kimi o Grok). Analitza el codi i comprova si les mesures de seguretat preses (vitest, CSP, preconnect, eliminació de window.location.pathname, on delete set null) són suficients per obrir l'app demà en Beta. Proposa qualsevol millora de puresa (code smells).

`EXECUTA: Analitza el bundle, verifica el tancament de les bretxes crítiques, i dóna llum verda o groga (amb propostes de puresa) per a la beta.`
...
- PROHIBICIÓ ESTRICTA DE CERCA WEB: Tota la documentació i el codi estan en el bundle adjunt. TENS PROHIBIT cercar termes (com oauthRelay.js, schema.sql o Pedra Seca) a internet. Ets en un entorn aïllat.
