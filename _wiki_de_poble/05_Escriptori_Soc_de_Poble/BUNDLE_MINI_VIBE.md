# BUNDLE MINI PER A L'AUDITORIA
Ací tens exclusivament els fitxers sol·licitats per a resoldre el problema de l'UI i la plantilla.
...
function NotesSectionContent() {
  ...
  return (
    <UniversalPage
      ...
      showLogos={true}
    >
      <article className={`notes-shell mobile-panel-${mobilePanel}`} data-editant={mobilePanel === 'editor'}>
        <NotesSidebar />
        <NotesList />
        <NotesEditor />
      </article>
...
import { Image as ImageIcon, Lock, Globe, FileText, Pencil } from 'lucide-react';
...
export default function NotesEditor() {
  const { activeNote, saveNoteField, noteFolders, t } = useNotes();
  ...
  if (!activeNote) {
    return (
      <section className="notes-column notes-column--editor no-scrollbar">
        <div className="chat-empty flex-col-center h-full">
          <FileText size={64} />
          <h2 className="section-title mt-4">{t('section.notes.open', 'Obre un solc')}</h2>
        </div>
      </section>
    );
  }
    ...
    <section className="notes-column notes-column--editor no-scrollbar">
      ...
      <div className="editor-scroll-area">
        ...
        <section className="bar-orange" aria-label="Autoria i data">
          ...
          <article className="card universal-page page-article border-none shadow-none sdp-mt-0">
            <header className="page-title border-bottom-none m-0">
              {!activeNote.headerImage ? (
                <div className="mb-6 sdp-mt-4" style={{ display: 'flex', justifyContent: 'center' }}>
                  <button type="button" className="pill dashed-pill pointer">
                    <ImageIcon size={16} /> Inserir Imatge o Multimèdia
                  </button>
                </div>
              ...
              <ul className="sp-card-labels page-title-labels flex-wrap">
              ...
              <p className="sp-card-copyright page-title-copyright text-center">
...
```
## Fitxer: src/sections/notes/NotesList.jsx
```jsx
import { useNotes } from './NotesContext';
...
export default function NotesToolbar({ editor }) {
  const { publishNote, activeNote, setMobilePanel } = useNotes();

  return (
    <div className="editor-toolbar">
      <div className="toolbar-actions">
        <div className="toolbar-group d-mobile-only">
          <button 
        ...
        <div className="toolbar-divider d-mobile-only"></div>
      ...
      <div>
        <button 
          ...
          onClick={publishNote} 
          className={`btn-publish ${!activeNote?.isPublished ? 'unpublished' : ''}`}
        >
...
```
## Fitxer: src/sections/notes/NotesSidebar.jsx
```jsx
import { useNotes } from './NotesContext';
  ...
  return (
    <aside className="notes-column notes-column--left no-scrollbar">
      <div className="notes-column-header">
        ...
        <button onClick={() => setColFoldersCollapsed(true)} className="btn-icon btn-icon--transparent" title="Replegar Columna">
          <PanelLeftClose size={18} />
        </button>
      </div>
...
@layer reset, tokens, base, components, notes, utilities;
...
nav.app-sidebar {
  ...
  flex-direction: column;
  height: 100%;
  background: var(--sdp-pedra-900);
  color: var(--sdp-pedra-50);
  position: relative;
...
.nav-item:hover { background: rgba(255,255,255,0.09); color: var(--sdp-pedra-50); }
...
.iaia-icon {
  fill: var(--sdp-accent); /* Orange color to match the old IAIA circular avatar */
  stroke: none;
...
.sw-pedra-75 { background: var(--sdp-pedra-100); color: var(--sdp-pedra-900); box-shadow: inset 0 0 0 1px rgba(0,0,0,0.05); }
...
.chat-bubble-ia { background-color: var(--sdp-fons-targeta); border: 1px solid var(--sdp-vora-control); border-radius: var(--sdp-radi-g) var(--sdp-radi-g) var(--sdp-radi-g) 0; padding: var(--sdp-space-3) var(--sdp-space-4); }
...
.chat-bubble-jl { background-color: var(--sdp-accio-text); color: var(--sdp-text-invers); border-radius: var(--sdp-radi-g) var(--sdp-radi-g) 0 var(--sdp-radi-g); padding: var(--sdp-space-3) var(--sdp-space-4); }
...
.login-layout {
  display: flex; flex-direction: column; align-items: center; gap: var(--sdp-space-5); margin-top: var(--sdp-space-4); margin-bottom: var(--sdp-space-6);
}

.login-switcher {
  display: flex; justify-content: center; gap: var(--sdp-space-3); background: var(--sdp-fons-targeta); padding: 8px; border-radius: 20px; box-shadow: var(--sdp-ombra-1); flex-wrap: wrap;
}

.pill--active {
  background-color: var(--sdp-accio); color: var(--sdp-sobre-accio);
}
...
.app-main__content article.card:has(.notes-shell) .topbar {
  position: static; border-radius: 0; border-width: 0; background: var(--sdp-fons-roca); color: var(--sdp-sobre-roca);
}

.app-main__content article.card:has(.notes-shell) .topbar__title, .app-main__content article.card:has(.notes-shell) .topbar__title strong {
  color: var(--sdp-sobre-roca);
}

.app-main__content article.card:has(.notes-shell) .topbar__actions {
  gap: 6px;
}
...
.sdp-tiptap-menu {
  ...
  flex-wrap: wrap;
  padding: var(--sdp-space-2);
  background: var(--sdp-fons-clar, var(--sdp-pedra-100));
  border-radius: var(--sdp-radi-s, 8px);
}

.sdp-tiptap-container {
  background: var(--sdp-fons-principal, var(--sdp-blanc-pur));
  border-radius: var(--sdp-radi-s, 8px);
}

.sdp-tiptap-container .ProseMirror {
  min-height: 300px;
  padding: var(--sdp-space-4);
  border: 1px solid var(--sdp-vora, var(--sdp-border-clar));
  border-radius: var(--sdp-radi-s, 8px);
  outline: none;
}

.sdp-tiptap-container .ProseMirror:focus {
  outline: 3px solid var(--sdp-focus);
  outline-offset: 1px;
}
```
...
- [[00_INDEX_ESCRIPTORI|05_Escriptori_Soc_de_Poble/00_INDEX_ESCRIPTORI.md]] — [[05_Escriptori_Soc_de_Poble/BUNDLE_MINI_VIBE|BUNDLE_MINI_VIBE]] — BUNDLE MIN...
