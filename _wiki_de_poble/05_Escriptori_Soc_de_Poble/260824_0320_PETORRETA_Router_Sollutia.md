...
tipus: "petorreta"
data: "2026-08-24"
abast: "Dilema Arquitectònic: HashRouter vs BrowserRouter per a la integració amb Sollutia"
---
# PETORRETA AL CONSELL: El Dilema del Router (HashRouter vs BrowserRouter)

## Context del Problema
Benvolgut Consell, us convoquem hui per a resoldre un dilema arquitectònic crític per a la integració del sistema front-end Pedra Seca amb la maquinària tècnica propietària de l'Agència Sollutia (Backend Privat).

Actualment, el codi utilitza `HashRouter` (les URL porten el símbol `#`, per exemple: `socdepoble.org/#/pobles`). Açò es va fer per a evitar dependències de configuració de reescriptura del servidor (Apache/Nginx/htaccess), garantint un "zero-friction deployment" en qualsevol entorn. Això és particularment útil perquè no tenim accés ni visibilitat del backend o l'arquitectura de servidors de Sollutia. Nosaltres ens enfoquem exclusivament en el front-end aïllat en Shadow DOM (cosa que hem testejat empíricament construint un plugin de WordPress paral·lel per validar l'aïllament).

## El Dilema
D'altra banda, `BrowserRouter` oferix URLs netes (`socdepoble.org/pobles`) que són el camí recomanat i estàndard, sobretot pel que fa al SEO i a la indexació per part de cercadors. Si fem el canvi a `BrowserRouter`, demanarà configuració del servidor per a reescriure totes les peticions al punt d'entrada de la SPA. Si ho fem amb `HashRouter`, funciona de la caixa però pot tindre un cost desconegut per al SEO.

## Preguntes a Resoldre:
1. Quin és l'impacte SEO real a dia de hui de tindre un sistema SPA carregat darrere del `HashRouter` dins d'un Backend Privat per a contingut que ha de ser indexable? 
2. Considerant que els recursos tècnics de Sollutia per a mantindre la infraestructura ens són desconeguts, i que s'ha de complir la "Llei de l'Enxufabilitat" (zero fricció), compense el risc i demanem que configuren el servidor per al `BrowserRouter`, o hem de mantindre el `HashRouter` i delegar el SEO al SSR de la seua plataforma?
3. Si recomaneu el `BrowserRouter`, quina serà l'estratègia tècnica d'integració més elegant per demanar als enginyers de Sollutia sense envair el seu territori?

A continuació, s'adjunta absolutament TOT el context de l'aplicació, cultura, identitat i codi font per a poder emetre un veredicte fonamentat, sense al·lucinacions.

---
**CONTEXT GLOBAL DE L'APLICACIÓ (SOSP_MASTER_CONTEXT_PACK)**
