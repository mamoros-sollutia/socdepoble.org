...
- **[2026-09-13] Auditoria de Reforç "Plantilla Enxufable" (Gemini Flash):**
  - Resultat: Ha posat el focus completament en el rendiment, el consum de recursos i l'estàndard Baseline 2022. Ha caçat dos problemes gravíssims que les altres IAs havien passat per alt a nivell d'execució profunda: (1) L'ús de `DOMParser` instanciat 100 vegades per tecla ofega els dispositius mòbils, i (2) L'ús de `:has()` a `AppGridShell.css` trenca el Baseline 2022. A més a més ha caçat exactament el punt de ruptura del filtre d'arbres al llistat amb la carpeta "Tot".
