...
A Petorreta is an audit prompt intended for external LLMs (Claude, GPT, etc.).
...
- **RELEVANT, VERIFIABLE CONTEXT**: Never bundle the entire Wiki by default. Create a selective context manifest containing only relevant sources, reasons, classifications and roles. If an external LLM needs source text, generate a redacted annex from that manifest.
