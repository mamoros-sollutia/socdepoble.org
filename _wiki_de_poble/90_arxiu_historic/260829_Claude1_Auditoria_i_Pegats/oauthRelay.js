...
const RELAY_PER_DEFECTE = 'https://auth.socdepoble.cat/callback';
...
export async function entraAmbGoogle(config = {}, resolConfig) {
  ...
  const emergent = window.open(url, 'sdp-oauth', 'width=520,height=680,noreferrer');

  // Camí 3: emergent bloquejat. Redirecció completa; la tornada la recull
  // `gestionaTornada()` quan l'app es torne a muntar.
  if (!emergent || emergent.closed) {
...
function esperaCodi(emergent, config) {
  return new Promise((resol, rebutja) => {
    ...
    function perMissatge(e) {
      if (e.origin !== window.location.origin && e.origin !== origenRelay) return;
...
export async function gestionaTornada(config = {}, resolConfig) {
  ...
  const codi = frag.get('sdp_code');
  const error = frag.get('sdp_oauth_error');
  ...
  return bescanvia(codi, verificador, { supabaseUrl, supabaseAnonKey });
}

function netejaFragment() {
  const net = window.location.pathname + window.location.search;
  window.history.replaceState(null, '', net);
}
