...
tipus: petorreta
estat: esborrany
description: "Auditoria estratègica per a la Fase 4: Supabase i TipTap"
---
# 🛡️ PETORRETA AL CONSELL: FASE 4 - SUPABASE I TIPTAP EXTENSIONS
...
Definir l'arquitectura per connectar l'Editor amb Supabase (Sessió i Storage) i dotar TipTap d'extensions riques (Slash Menu, Embeds).

`OBJECTIU: Definir l'arquitectura d'integració de Supabase i extensions TipTap sense trencar la 'Caixa Única'`

## Context Necessari

Bundle aparellat: 260913_0420_BUNDLE_auditoria.md.

Acabem de finalitzar la **Fase 3**. L'`UniversalEditorShell` ha assolit la independència arquitectònica: l'host (ex: `NotesEditor`) ara munta el `PageFrame` i injecta la closca. El contracte CSS de la 'Caixa Única' (ues-root, ues-scroll) impedeix pantalles blanques, i el protocol de commit amb `visibilitychange` evita pèrdua de dades.

No obstant, l'usuari reporta que en intentar pujar la foto de perfil falla amb l'error `"No hi ha sessió"`. A més, el motor TipTap (`useUniversalRichText`) és pobre i només compta amb l'StarterKit. Ens preparem per a la **Fase 4**.

## Instrucció Principal

Analitza el bundle i proposa la millor estratègia d'integració, actuant com a Consell Assessor.

`EXECUTA: Analitza el codi i proposa estratègies concretes`

Responeu a aquestes dues qüestions clau:
1. **El Repte de Supabase i Storage:** Actualment falla per "No hi ha sessió" en editar el perfil (probablement mock vs auth real). Com integraríeu la sessió d'Auth i la pujada d'imatges a Supabase (Storage) respectant que `UniversalEditorShell` ha de seguir sent un component endollable (plugin) i agnòstic del backend directe?
2. **El Repte de TipTap Rics:** Com afegiríeu les capacitats rich-text (menú 'Slash' flotant per inserir imatges, vídeos o divisor) a la nostra arquitectura modular (`UniversalRichTextContent` / `useUniversalRichText`) sense introduir col·lapses CSS ni trencar el contracte de la Caixa Única?
