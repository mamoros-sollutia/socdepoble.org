...
Auditoria Tècnica i Funcional de l'arquitectura de la `UniversalPage`, la Targeta Mestra, i la seua aplicació al `NotesEditor` (Bloc de Notes). Volem assegurar que el codi compleix de forma estricta amb l'estàndard "Pedra Seca", la taxonomia de Píndoles (Carpetes, Categories, Etiquetes) i el comportament del Hero Image.

## Context Necessari

Bundle aparellat: `260904_0106_BUNDLE_auditoria.md` (Aquest arxiu conté tota la veritat neta del sistema, llig-lo amb atenció).
La integració actual és online i Sollutia-first. Revisa especialment la *SKILL* `universal-page` per entendre les regles inviolables.

## Instrucció Principal

Analitza el codi de `NotesEditor.jsx`, `NotesContext.jsx`, `UniversalComponents.jsx` i `DesignSection.jsx`.
1. Verifica que la lògica de les píndoles (`sdp-badge-system`, `sdp-badge-category`, `sdp-badge-tag`) està ben implementada i no genera codi o divs innecessaris.
2. Comprova la usabilitat i el codi dels nous botons del Hero Image ("Inserir Imatge", "Tornar enrere", "Esborrar contingut").
3. Detecta qualsevol "AI slop" o codi redundant (Tailwind, classes no semàntiques) i proposa la poda.

## Output Esperat

Vull un informe clar i directe, sense "yapping". Llista els problemes trobats i el codi exacte per a solucionar-los seguint la filosofia "Escriu la porta, no el comentari".
