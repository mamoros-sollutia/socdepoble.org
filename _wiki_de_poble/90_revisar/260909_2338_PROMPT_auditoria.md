...
Auditar el sistema per a trobar vulnerabilitats de la Fase 6 (RLS i Wrapper d'autenticació) i fer una neteja profunda ("salfumán") d'elements fantasma a la UI.

`OBJECTIU: Detectar riscos en RLS (Fase 6) i netejar el frontend de divs i classes CSS orfes.`

## Context Necessari

Bundle aparellat: 260909_2338_BUNDLE_auditoria.md.

La integració actual és purament online i centrada en crear una connexió perfecta per al sistema de la nostra empresa sòcia, Sollutia; la sobirania local és una meta de llarg termini. Estem preparant el terreny per a la Fase 6 (Seguretat) i futures fases (Gestoria, Llibreria), pel que exigim un codi base atòmic i completament net.

## Instrucció Principal

1. **Fase 6 (RLS i Auth):** Identifica vulnerabilitats en la capa de dades i proporciona propostes concretes per implementar les polítiques de seguretat pendents a Supabase i el wrapper d'autenticació al frontend.
2. **Neteja amb Salfumà (Fantasmes UI):** Escaneja rigorosament tot el codi React i `index.css`. Llista qualsevol classe CSS que estiga definida i no s'use, o classes que s'usen i no existixen. Identifica `div`s inútils que trenquen la semàntica o compliquen la graella. Necessitem un diagnòstic destructiu per deixar la UI immaculada i escalable abans de continuar creixent.

`EXECUTA: Analitza el codi, assenyala vulnerabilitats per a la Fase 6 i llista de forma exhaustiva la brossa CSS/HTML (fantasmes) per eliminar-la.`

## Output Esperat

`FORMAT: markdown. Per a la part del "Salfumà" (Neteja CSS/HTML), presenta els elements a eliminar OBLIGATÒRIAMENT en forma de taula o llista exacta (Nom del Fitxer | Classe o Div a esborrar | Motiu). Evita explicacions llargues, vés al gra perquè una altra IA puga automatitzar la neteja.`
