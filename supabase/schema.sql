...
-- ---------------------------------------------------------------------------
-- Personal identity and organizations
-- ---------------------------------------------------------------------------
-- Personal profiles stay private. Companies and groups are separate tenant
-- resources. Authenticated writes run with caller privileges and remain under
-- the RLS policies below, including when they arrive through the RPC.
...
create table if not exists public.organizations (
  ...
  check (
    (kind in ('company', 'entity', 'city_hall') and parent_organization_id is null)
...
-- SDP-LOCK: Protegeix contra l'eliminació de l'últim propietari
...
-- Public directory deliberately omits created_by and membership rows.
...
drop function if exists public.create_organization(uuid, text, text, text, text, uuid);
...
revoke execute on function public.create_organization(uuid, text, text, text, text, uuid) from public;
revoke execute on function public.create_organization(uuid, text, text, text, text, uuid) from anon;
grant execute on function public.create_organization(uuid, text, text, text, text, uuid) to authenticated;
