...
// The file is mostly a huge JSON object. 
// We can use a regex to match the "skills" block.
// "skills": { ... "html": "..." }

let result = content.replace(/("skills":\s*\{\s*"date":\s*")[^"]+("[^}]*?"html":\s*")[^"]+("\n\s*\})/, (match, p1, p2, p3) => {
  return p1 + "2026-09-11T00:00:00.000Z" + p2 + newSkillsHtml + p3;
});
