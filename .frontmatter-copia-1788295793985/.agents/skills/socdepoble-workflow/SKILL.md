...
lang: ca
core: false
triggers_on: ["workflow", "flux", "procés", "passos", "guia", "socdepoble workflow"]
---
...
5. **Ancoratge de Seguretat (Zero Satèl·lits):** Qualsevol arxiu (Markdown, script, imatge, etc.) que generes i guardes a l'Escriptori o a la Wiki HA DE REBRE IMMEDIATAMENT un "Ancoratge de Seguretat". Això vol dir que has de registrar el seu enllaç (`[[Nom_del_Fitxer]]`) a l'índex corresponent (per exemple, `00_INDEX_ESCRIPTORI.md`). Mai pots crear un fitxer solitari sense ancorar-lo; això genera "satèl·lits" invisibles a Obsidian. Grava-t'ho com a instint bàsic.
...
7. **Safata d'Entrada Neta (Zero Inbox):** La `00_Bandeja_d_Entrada` ha d'estar sempre neta. Si una IA, un procés o un humà deixa arxius ací (com un bundle), és responsabilitat teua moure'ls directament a l'arrel de l'Escriptori (`05_Escriptori_Soc_de_Poble/`) o a `01_Produccio` perquè el Mestre valore on van. Mai deixes arxius abandonats a la safata d'entrada, i mai tanques una sessió sense assegurar-te d'haver netejat la brutícia residual.
