...
import { dashboardDetalls } from "./dashboard_data.js";
...
export async function processUnknownCSV(csvText) {
...
export async function initializeDBWithCSV() {
    try {
        ...
        if (countBanc < 10) { // Menys de 10 significa que o està buida o només té les dades falses de demo
            ...
            try {
                ...
                if (typeof dashboardDetalls !== 'undefined') {
                    const eventsToInsert = [];
                    const contactesMap = new Map();
                    
                    Object.keys(dashboardDetalls).forEach(category => {
                        dashboardDetalls[category].forEach(item => {
