...
- El `host.js` actua de pany. Abans de ser cridada la funció `arrenca()`, l'entorn (Sollutia, WordPress, etc) injecta la seua implementació. Una vegada segellat, és immutable.
...
- A `PedraSecaEmbed.jsx`, s'utilitza `window.__SDP_REACT_MOUNTED__` com a pany (lock) global per evitar la corrupció i els zombis de React per culpa de l'editor de blocs de WordPress.
